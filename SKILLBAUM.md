...
|---|---|---|
| 2026-09-04 | Dritter Vertikalschnitt: Musizieren | **Der erste Schnitt ausserhalb von `sport` - und damit der Beleg, dass das Muster nicht nur zweimal derselbe Fall war.** `AvatarActivityPlans.resolve` bleibt der einzige Eingang; dazu kam eine dritte Verzweigung und `resolveMusic`. **Wieder kein neuer RoutineStep, keine neue PlayEffects-Phase** - `MusicPhase{TUNE,PLAY,FINALE}` und `RoutineStep.Music` gab es laengst, `PlayRoutines` benutzt sie sogar schon in der Ambient-Routine "Musizieren im Park"; gefehlt hat nur die Verbindung zur Absicht. **Der Beweis liegt im Themen-Typ:** Sport meldet MOVE, Musik meldet CREATIVITY - beides aus dem Baum abgelesen, nirgends selbst gewaehlt. `TUNE`+`PLAY` sind die Basis fuer jeden (Stimmen allein waere ein Stumpf), `FINALE` nur mit tatsaechlich freigeschaltetem `singen`. `drum` und `bolt` bleiben bewusst draussen: keine Phase, die sie unterscheiden koennte. Orte: Park, Wiese, Wohnzimmer, Leseecke sind lokal - anders als beim Sport, denn ein Instrument braucht kein Feld; sonst `GoToPlace(PARK)`. **Nebenbefund:** `kreativ/musik` und `.../singen` standen im Duplikat-Bericht als identisches Paar - jetzt unterscheidbar, ohne eine Zeile Reaktionscode. **Neu im Werkzeugkasten:** `tools/reaction-preview/tests.sh` fuehrt 94 Unit-Tests wirklich AUS (bisher konnten die Werkzeuge nur uebersetzen), unter einer Sekunde, ohne Gradle und Geraet. **Fuer die naechste Sitzung:** `tests.sh` VOR dem Push laufen lassen. Unangeschlossen warten noch `BasketballPhase`, `PaintingPhase`, `FishingPhase`, `KitePhase`. |
| 2026-09-04 | Zweiter Vertikalschnitt: Kraft & Ausdauer | **Das Muster aus dem Fussball-Schnitt traegt einen zweiten Fall.** `AvatarActivityPlans.resolve` bleibt der einzige Eingang; dazugekommen sind eine Verzweigung nach der Wirtsbeschaeftigung und ein zweiter Schritt-Bauer (`resolveStrength`). **Kein neuer RoutineStep, keine neue PlayEffects-Phase** - `TrainingPhase{WARM_UP,LIFT,REST}` und `RoutineStep.Training` gab es laengst, gefehlt hat nur die Verbindung zur Absicht. `WARM_UP` ist die Basis fuer jeden (Gegenstueck zu `TOUCH`), `LIFT` nur mit tatsaechlich freigeschaltetem `heben`, `REST` immer - ein Abschluss ist kein Koennen. **Dabei ein Fehler behoben, den auch der Fussball trug:** Eine Einlage in eine bereits laufende Beschaeftigung stellte Hinweg, Anker und Basisphase erneut voran - wer beim Ballspielen ein Dribbling bekam, sah ihn wieder hinlaufen und den Ball erstmals beruehren. `planFor` sagte das Richtige (nur `Flourish`, kein `Begin`), die Ausfuehrung ignorierte es. Die Regel liegt jetzt gemeinsam in `isInsert` statt zweimal. Gemeldet von Codex an der Kraft-Familie. **Neu im Werkzeugkasten:** `tools/reaction-preview/routines.sh` gibt die erzeugte Schrittfolge je Ort und Freischaltungsstand aus. **Fuer die naechste Sitzung:** zuerst nachsehen, was schon unangeschlossen bereitliegt - `BasketballPhase`, `MusicPhase`, `PaintingPhase`, `FishingPhase`, `KitePhase`. |
