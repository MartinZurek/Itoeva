...
1. **NT-066** - Read-only Snapshot/Event-Quelle und Mehrtages-Langlauftest ergaenzen.
...
2. **NT-058** - Begrenzten End-to-End-PoC fuer genau einen oeffentlichen Charakter durchfuehren:
...
5. **NT-047** - Standard-Modellwahl je Aufgabentyp pruefen. *Auswahl begruendet dokumentiert oder
   angepasst.* (keine; 45 min)
6. **NT-005** - Gradle-Abhaengigkeits-Caching in den Hauptworkflows pruefen. *Gemessene
   Laufzeitwirkung.* (keine; 60 min)
7. **NT-004** - Timeout-Werte aller Jobs pruefen und ergaenzen. *Jeder Job hat einen begruendeten
    Grenzwert.* (keine; 30 min)
8. **NT-002** - Klaeren, ob `runner/` noch gebraucht wird. *Doppelte Pflege ist begruendet
    beendet oder dokumentiert.* (menschliche Freigabe fuer Entfernen; 60 min)
9. **NT-029** - `README.md` in kurzen Einstieg und Themendokumente aufteilen. *Unter 300
    Zeilen, keine verlorenen Inhalte oder toten Links.* (keine; 90 min)
10. **NT-030** - Kurze Zustaendigkeitsnotiz je Gradle-Modul ergaenzen. *Drei Dateien unter je 50
    Zeilen.* (keine; 60 min)
11. **NT-009** - Flaky-Rate des Emulator-Jobs ueber die letzten 20 Laeufe messen. *Kennzahl und
    gegebenenfalls neue Schwelle dokumentiert.* (keine; 60 min)
12. **NT-050** - Tokenverbrauch der letzten zehn Evolutionslaeufe als Baseline dokumentieren.
    *Zahl, Quelle und Datum stehen in der Architektur-/Evolutionsdokumentation.* (keine; 45 min)

13. **NT-007** - Die durchschnittlichen Minutenkosten der letzten zehn abgeschlossenen
...
14. **NT-008** - Die API-Level-Matrix gegen `minSdk` und `targetSdk` pruefen. *Unterstuetzte
    Geraete, instrumentierte CI-Abdeckung und bewusst offene Luecken sind dokumentiert.*
    (keine; 45 min)

15. **NT-010** - PLAY_STORE.md-Signierungsdokumentation gegen den `release`-Job abgleichen.
...
**Build/CI, weitere:** NT-011 Selbsttest für `backlog-select.sh` · NT-012 Gradle/AGP-
